...
function permuteKey(key, permutationTable, roundNumber) {
    ...
    leftKey = circularShiftLeft(leftKey, shiftTable[roundNumber - 1]);
    
    rightKey = circularShiftLeft(rightKey, shiftTable[roundNumber - 1]);
    
    combinedKey = leftKey + rightKey;
...
function f(right, key, round) {
    ...
    let xor = xOr(expandedRight, roundKey);
    
    let result = '';

    for (let i = 0; i < 8; i++) {
        let block = xor.slice(i * 6, (i + 1) * 6);
        let row = parseInt(block[0] + block[5], 2);
        let col = parseInt(block.slice(1, 5), 2);
        result += ('0000' + sBoxes[i][row * 16 + col].toString(2)).slice(-4);
    }
